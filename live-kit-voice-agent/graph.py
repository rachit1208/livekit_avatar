...
def create_workflow():
    ...
    pdf_path = os.getenv(
        "COMPANY_PDF_PATH", "SysTools Corporate Profile.pdf"
    )
    if not os.path.exists(pdf_path):
        raise FileNotFoundError(
            f"PDF file not found: {pdf_path}. Please set COMPANY_PDF_PATH environment variable or place TechCompanyInfo.pdf in the current directory."
        )
    ...
    graph.add_conditional_edges(
        "llm", decide_next_action, {"tool_executor": "tool_executor", "end": END}
    )
